...
public class OpenAccountController {
    ...
    public ResponseEntity<OpenAccountResponse> openAccount(@Valid @RequestBody OpenAccountCommand command){
        ...
        try {
            ...
            return new ResponseEntity<>(new OpenAccountResponse(id, "succesfully opened a nem bank account!"), HttpStatus.CREATED);
        }catch (Exception e){
            String safeErrorMessage="Error while processing request to open a nem bank account for id -" +id;
